...
constexpr RoomId    LOBBY_ROOM_ID = 1;   // 로비 Room ID
...
private:
    ...
    std::unordered_map<RoomId, uint16_t> room_to_worker_;
...
public:
    ...
    uint16_t bind_room_worker(RoomId room_id);
    void erase_room_to_worker(RoomId room_id);
    void disconnect_session(int fd, RoomId room_id);

    void request_flush(std::shared_ptr<Session> session);

private:
    uint16_t find_worker(RoomId room_id);
